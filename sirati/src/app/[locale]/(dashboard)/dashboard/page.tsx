  ...
  useEffect(() => {
    async function load() {
      ...
      if (user) {
        ...
        if (data) setStats({ total: data.length, lastScore: data[0]?.match_score || 0 });
      }
    }
    load();
  }, []);
    ...
    <div style={{ padding: '40px 48px', maxWidth: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 40 }}>
        {[
          ...
          <div key={stat.label} style={{ background: '#fff', borderRadius: 16, padding: '24px 28px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0071e3, #2997ff)', borderRadius: 20, padding: '36px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
        ...
        <Link href={p('/generate')} style={{ background: '#fff', color: '#0071e3', padding: '14px 28px', borderRadius: 12, fontWeight: 700, fontSize: 15, textDecoration: 'none', whiteSpace: 'nowrap', marginLeft: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, marginTop: 24 }}>
        {[
          ...
          <Link key={item.href} href={item.href} style={{ background: '#fff', borderRadius: 16, padding: '24px 28px', textDecoration: 'none', display: 'block', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
